---
title: How It Works
description: "Core concepts about blockchain traceability and the Winter protocol"
order: 0
---
...
However, instead of minting a new NFT, the same NFT will be used, but transferred to a new UTxO. This updates the on-chain data reference to the new CID while keeping the same token identifier.

## Delete

Once traceability data is no longer needed, it is possible to also delete the on-chain reference. This is done by spending the UTxO, which burns the NFT and removes the spendable reference. The transaction history remains on chain.
